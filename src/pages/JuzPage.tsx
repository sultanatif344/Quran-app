...
export function JuzPage() {
  ...
  useEffect(() => {
    ...
    if (ayahParam) {
      ...
      const idx = pageIndexOf(data, s, a)
      setPageIdx(idx)
      setHighlight(`${s}:${a}`)
      setParams({}, { replace: true })
      requestAnimationFrame(() =>
        document.getElementById(`ayah-${s}-${a}`)?.scrollIntoView({ block: 'start', behavior: 'auto' }),
      )
    } else if (pageParam >= 1) {
  ...
  return (
    <>
      <AppHeader
        title={
          <>
            پارہ {toUrduDigits(juz)} <span lang="ar" style={{ fontWeight: 400 }}>{juzName(juz)}</span>
          </>
        }
      ...
      <main className="page page--reader">
        ...
        {data && page && (
          <LafziPage
            ...
            onToggleBookmark={(s, a) => toggleBookmark({ juz, surah: s, ayah: a })}
          />
        )}
      </main>
      ...
      <nav className="bottom-bar" aria-label="پڑھنے کے اختیارات">
        <div className="bottom-bar__inner">
          <button className="btn btn--nav" disabled={!hasPrev && !prevJuz} onClick={goPrev} aria-label="پچھلا صفحہ">
          ...
          <button className="btn" onClick={() => setGotoOpen(true)} disabled={!data} aria-label="صفحہ نمبر پر جائیں">
            صفحہ
          </button>
          <button className="btn" onClick={() => stepFont(1)} disabled={settings.fontStep === MAX_STEP} aria-label="حروف بڑے کریں">
            <span className="latin">A+</span>
          </button>
          <button className="btn btn--nav" disabled={!hasNext && !nextJuz} onClick={goNext} aria-label="اگلا صفحہ">
